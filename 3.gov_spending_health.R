...
View(africa_only_long)

unique(africa_only_long$GeoAreaName)
# Combine Africa region with African Continent ----------------------------

regional_perc_gove_health_exp_all <- bind_rows(
  africa_long_region,
  africa_only_long
)
# Regional median values by country and year ------------------------------

regional_perc_gov_health_exp = regional_perc_gove_health_exp_all %>%
...
# regional summaries  ---------------------------------------------------
region_medians <- regional_perc_gov_health_exp %>%
...
regional_perc_gove_health_exp_labeled <- regional_perc_gov_health_exp %>%
...
regional_perc_gov_health_exp <- regional_perc_gov_health_exp %>%
...
sub_region_plot <- ggplot() +
  geom_line(
    data = regional_perc_gov_health_exp %>% filter(legend_group == "Sub-Region"),
...
reference_plot <- ggplot() +
  geom_line(
    data = regional_perc_gov_health_exp %>% filter(legend_group == "Reference"),
    aes(x = Year, y = regional_value_median, color = region),
    linewidth = 1.5
  ) +
  scale_color_manual(
...
View(regional_perc_gov_health_exp)
...
country_list = regional_perc_gove_health_exp_all %>%
...
ft <- set_header_labels(ft,
  ...
  # Set Title
  set_caption(ft, caption = "Table X. Median Percentage of Government Health by Country")

# Export to Word
doc <- read_docx()
doc <- body_add_flextable(doc, value = ft)
print(doc, target = paste0(out_fld_health,"/country_gov_expenditure_health.docx"))
